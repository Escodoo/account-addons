...
class SaleBlanketOrder(models.Model):
    ...
    def _compute_forecast_uninvoiced_amount(self):
        for order in self:
            amount_invoiced = sum(
                x.amount_total
                for x in order.line_ids.sale_lines.order_id.filtered(
                    lambda x: x.state in ["draft", "sent", "sale"]
                )
            )
    ...
    def _compute_amount_uninvoiced_company_currency(self, amount_uninvoiced):
        if (
            ...
            rate_date = self.validity_date or fields.Date.today()
    ...
    def write(self, values):
        res = super(SaleBlanketOrder, self).write(values)
        if any(
            [
                field in values
            ...
            for rec in self:
                if (
                    values.get("state") in ["draft", "open", "done"]
                    or rec.state in ["draft", "open", "done"]
                    and rec.company_id.enable_sale_blanket_order_mis_cash_flow_forecast
                ):
    ...
    def _generate_mis_cash_flow_forecast_lines(self):
        ...
        for rec in self:
            rec.mis_cash_flow_forecast_line_ids.unlink()
            if rec.forecast_uninvoiced_amount and rec.state in ["open", "done"]:
                sign = 1
