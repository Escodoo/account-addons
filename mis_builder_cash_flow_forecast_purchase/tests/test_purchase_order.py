...
class TestPurchaseOrder(TransactionCase):
    ...
    def test_prepare_mis_cash_flow_forecast_line(self):
        ...
        expected_account_id = purchase_order.partner_id.with_company(
    ...
    def _create_company_with_payable_account(self, name, code):
    ...
    def test_prepare_mis_cash_flow_forecast_line_multi_company(self):
        company_2, payable_account_2 = self._create_company_with_payable_account(
            "Company 2", "PAYC2"
        )
        purchase_order = self.env["purchase.order"].new(
            {"partner_id": self.partner.id, "company_id": company_2.id}
        )
        # Forecast lines are generated by the cron, whose environment company may
        # differ from the purchase order company
        result = purchase_order.with_company(
            self.company
        )._prepare_mis_cash_flow_forecast_line("1", 1, self.current_date, 100.0)
        self.assertEqual(result["account_id"], payable_account_2.id)
        self.assertEqual(result["company_id"], company_2.id)

    def test_prepare_mis_cash_flow_forecast_line_account_other_company(self):
        company_2, payable_account_2 = self._create_company_with_payable_account(
            "Company 2", "PAYC2"
        )
        company_3, payable_account_3 = self._create_company_with_payable_account(
            "Company 3", "PAYC3"
        )
        # Inconsistent data: in the order company, the partner payable account
        # belongs to another company
        self.env["ir.property"].create(
            {
                "name": "property_account_payable_id",
                "fields_id": self.env["ir.model.fields"]
                ._get("res.partner", "property_account_payable_id")
                .id,
                "res_id": f"res.partner,{self.partner.id}",
                "value_reference": f"account.account,{payable_account_3.id}",
                "type": "many2one",
                "company_id": company_2.id,
            }
        )
        self.assertEqual(
            self.partner.with_company(company_2).property_account_payable_id,
            payable_account_3,
        )
        purchase_order = self.env["purchase.order"].new(
            {"partner_id": self.partner.id, "company_id": company_2.id}
        ...
        result = purchase_order._prepare_mis_cash_flow_forecast_line(
            "1", 1, self.current_date, 100.0
        )
        # Falls back to the payable account of the order company partner
        self.assertEqual(result["account_id"], payable_account_2.id)
        self.assertNotEqual(result["account_id"], payable_account_3.id)
